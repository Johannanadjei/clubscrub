...
function AssistantSignup({ onComplete }) {
  ...
  const steps = [
    ...
    <FadeUp key="0">
      ...
      <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.72)', fontWeight: 300, marginBottom: 24 }}>Become a ClubScrub Assistant and earn on your schedule.</p>
      ...
      {[
        ...
        <div key={k} style={{ marginBottom: 14 }}>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>{label}</p>
    ...
    <FadeUp key="1">
      ...
      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Available days</p>
      ...
      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Skills</p>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>About you</p>
    ...
    <FadeUp key="2">
      ...
      <div className="cs-card p-5 mb-4" style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', marginBottom: 16, fontWeight: 300 }}>Profile photo</p>
      ...
      <div className="cs-card p-5 mb-4" style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', marginBottom: 16, fontWeight: 300 }}>Ghana Card / Passport</p>
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 300 }}>{job.area} · {job.distance}</span>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <p className="font-display italic" style={{ fontSize: 20, fontWeight: 600, color: '#EC2461' }}>GH₵ {job.pay}</p>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)' }}>Est. pay</p>
        </div>
      </div>
      <div className="flex items-center gap-4 mb-3">
        <div className="flex items-center gap-1.5">
          <Clock size={12} style={{ color: 'rgba(255,255,255,0.6)' }} />
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 300 }}>{job.timeSlot}</span>
        </div>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 300 }}>{job.date}</span>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
        {job.tasks.map(t => (
          <span key={t} style={{ background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 7, fontSize: 11, padding: '3px 9px', color: 'rgba(255,255,255,0.65)' }}>{t}</span>
        ))}
      </div>
      {accepted ? (
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>{profile.jobs} jobs</span>
          </div>
        </div>
      ...
      {tab === 'jobs' ? (
        ...
        acceptedJobs.length === 0 ? (
          <div className="cs-card p-8 text-center">
            ...
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', fontWeight: 300 }}>No jobs accepted yet</p>
          </div>
        ) : acceptedJobs.map(j => (
          <div key={j.id} className="cs-card p-4 mb-3">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', fontWeight: 300 }}>{j.area} · {j.date}</p>
              </div>
            ...
            <div className="flex gap-2">
              ...
              {jobStatus[j.id] === 'started' && (
                <>
                  ...
                  <button style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '11px', fontSize: 12, color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                    Report Issue
                  </button>
                </>
              )}
